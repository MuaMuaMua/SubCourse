...
- (instancetype)initWithDataArr:(NSArray *)dataArr origin:(CGPoint)origin width:(CGFloat)width rowHeight:(CGFloat)rowHeight
{
    ...
    if (self) {
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake((kScreenWidth-width)/2, origin.y + TopToView, width, rowHeight * dataArr.count) style:UITableViewStylePlain];
...
{
    if([self.delegate respondsToSelector:@selector(SCTitleMenu:didSelectRowAtIndexPath:)]){
        [self.delegate SCTitleMenu:tableView didSelectRowAtIndexPath:indexPath];
    }
...
- (void)dismissWithCompletion:(void (^)(SCTitleMenu *object))completion
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.alpha = 0;
        weakSelf.tableView.frame = CGRectMake((kScreenWidth-125)/2, self.origin.y + TopToView, 0, 0);
...
- (void)drawRect:(CGRect)rect
{
    ...
    CGContextMoveToPoint(context,
                         kScreenWidth/2, 5);//设置起点
    
    CGContextAddLineToPoint(context,
                            kScreenWidth/2-10, 10);
